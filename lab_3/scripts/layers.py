...
def relu_forward(x):
    ...
    out = np.maximum(0, x) # поэлементно проверяем в массиве 
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    # определяем координаты окна во входном тензоре
                    h_start = i * stride
                    h_end = h_start + HH
                    w_start = j * stride
                    w_end = w_start + WW
                    
                    # извлекаем окно и выполняем свертку
...
def conv_backward_naive(dout, cache):
    ...
    # добавляем padding к входу и градиенту
    x_padded = np.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)), mode='constant')
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    # градиент по bias - сумма по пространственным координатам
                    db[f] += dout[n, f, i, j]
                    
                    # градиент по весам - произведение окна на градиент выхода
                    dw[f] += window * dout[n, f, i, j]
                    
                    # градиент по входу - распределение градиента через веса
                    dx_padded[n, :, h_start:h_end, w_start:w_end] += w[f] * dout[n, f, i, j]
    
    # убираем padding из градиента по входу
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    # извлекаем окно и берем максимум
