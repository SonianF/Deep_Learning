...
class KNearestNeighbor(object):
    ...
    def predict(self, X_test, k=5, num_loops=0):
        """
    ...
    def compute_distances_two_loops(self, X_test):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dists[i][j] = np.sqrt(np.sum(pow(X_test[i] - self.X_train[j], 2)))
    ...
    def compute_distances_one_loop(self, X_test):
        ...
        for i in range(num_test):
            ...
            dists[i, :] = np.sqrt(np.sum(pow(self.X_train - X_test[i], 2), axis=1))
    ...
    def compute_distances_no_loops(self, X_test):
        ...
        test_sq = np.sum(pow(X_test, 2), axis=1, keepdims=True)
  
        train_sq = np.sum(pow(self.X_train, 2), axis=1, keepdims=True).T
        
        # X * X_train^T, shape (num_test, num_train)
        cross = np.dot(X_test, self.X_train.T)
        
        dists_sq = test_sq + train_sq - 2 * cross
    ...
    def predict_labels(self, dists, k=5):
        ...
        for i in range(num_test):
            ...
            sort_idxs = np.argsort(dists[i])[:k]
            ...
            marks = np.bincount(closest_y)
            y_pred[i] = np.argmax(marks)
